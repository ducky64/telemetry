...
Record IDs 1-63 (0x01-0x7f) are reserved for common global records definitions. IDs from 64 onwards are free to be defined per data type.
...
Record ID 64: data length (in bytes)
\subsubsection{Data format}
Raw data in big endian format.

\subsection{Float: Data type 1}
\subsubsection{KV Records}
Record ID 64: data length (in bytes, only 4 ({\texttt float}) and 8 ({\texttt double}) are valid)
\subsubsection{Data format}
